...
export default function JioMartProductCard({ product, quantity, onAddToCart, onViewDetails }: ProductCardProps) {
  return (
    <div
      id={`jiomart-product-${product._id}`}
    ...
    >
      ...
      <div className="p-3">
        ...
        {quantity === 0 ? (
          ...
          <div className="flex items-center justify-between mt-3 bg-blue-100 rounded p-1">
            <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-blue-600">
              <Minus className="w-4 h-4" />
            </Button>
            <span className="font-semibold text-sm text-blue-600">{quantity}</span>
            <Button
              onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                e.stopPropagation()
                onAddToCart()
