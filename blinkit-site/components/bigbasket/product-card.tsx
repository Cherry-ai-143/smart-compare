...
export default function BigBasketProductCard({ product, quantity, onAddToCart, onViewDetails }: ProductCardProps) {
  return (
    <div
      id={`bigbasket-product-${product._id}`}
    ...
    >
      ...
      <div className="p-3">
        ...
        {quantity === 0 ? (
          ...
          <div className="flex items-center justify-between mt-3 bg-green-100 rounded p-1">
            <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-green-600">
              <Minus className="w-4 h-4" />
            </Button>
            <span className="font-semibold text-sm text-green-600">{quantity}</span>
            <Button
              onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                e.stopPropagation()
                onAddToCart()
